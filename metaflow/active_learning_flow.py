...
class ActiveLearningFlow(FlowSpec):
    ...
    def start(self):
        ...
        self.n_initial = 1000
        np.random.seed(1234)
        self.test_idx = np.random.choice(range(len(self.y_pool)), size=self.n_initial, replace=False)
        ...
        np.random.seed(1234)
        self.initial_idx = np.random.choice(range(len(self.y_pool)), size=self.n_initial, replace=False)
        ...
        self.initial_dataset = CustomDataset(self.X_initial, self.y_initial, transform=self.custom_transform)
        self.test_set = CustomDataset(self.X_test, self.y_test, transform=self.custom_transform)
        self.pool = CustomDataset(self.X_pool, self.y_pool, transform=self.custom_transform)
        self.batch_size = 32
        self.train_loader = DataLoader(self.initial_dataset, batch_size=self.batch_size, shuffle=True)
    ...
    def inital_training(self):
        ...
        acc, prec, rec, f1_score = evaluate(self.model_trained, self.test_loader, self.device)
        self.accuracy = [acc]
        self.precision = [prec]
        self.recall = [rec]
        self.f1 = [f1_score]
    ...
    def active_learning_loop(self):
        ...
        for j in range(self.n_iterations):
            ...
            self.indices, _ = UncertaintySampling(MarginMethod, self.device).get_samples(self.model_trained, 
                                                                                    self.pool_loader, 
                                                                                    self.n_instances)
            # Move instances from pool_loader to train_loader
            new_train_data = np.concatenate((self.train_loader.dataset.data, self.pool_loader.dataset.data[self.indices]), axis=0)
            new_train_labels = np.concatenate((self.train_loader.dataset.labels, self.pool_loader.dataset.labels[self.indices]), axis=0)
            ...
            self.train_loader = DataLoader(self.train_loader.dataset, batch_size=self.batch_size, shuffle=True)
            
            # Update the pool_loader to remove the transferred instances
            self.pool_loader.dataset.data = np.delete(self.pool_loader.dataset.data, self.indices, axis=0)
            self.pool_loader.dataset.labels = np.delete(self.pool_loader.dataset.labels, self.indices, axis=0)
