...
class Method(ABC):
    ...
    def get_score(self, *args, **kwargs):
        raise NotImplementedError("Implement the `get_score`")
...
class MarginMethod(Method):
    ...
    def get_score(self, sorted=False):
        ...
        """
        if not sorted:
            self._prob_dist, _ = torch.sort(
                self._prob_dist, descending=True
            )  # sort probs so largest is first

        self._prob_dist = self._prob_dist.reshape(-1)
        difference = (
            self._prob_dist.data[0] - self._prob_dist.data[1]
        )  # difference between top two props
